...
GapSegment::GapSegment(const Slice &slice, size_t &pos) {
  auto pos1 = pos;
  status = read_var_u32(slice, pos1, segStartIdxInsideInterval);
  if (!status.ok()) {
    return;
  }
  MergeFrom(slice, pos);
...
void GapSegment::WriteTo(std::string &buf) {
  write_var_u32(buf, segStartIdxInsideInterval);
...
void GapSegment::MergeFrom(const rocksdb::Slice &slice, size_t &pos) {
  ...
  if (intervalIdx < segStartIdxInsideInterval) {
    status = Status::Corruption(
        "segStartIdxInsideInterval of the merged segment is smaller than the "
        "one it is merged to: " +
        std::to_string(intervalIdx) + " < " +
        std::to_string(segStartIdxInsideInterval));
    return;
  }
  ...
  auto d = intervalIdx - segStartIdxInsideInterval;
  if (d > 0) {
